...
(function (SellerSigner) {
    async function generateUnsignedListingPSBTBase64(listing) {
        ...
        const tx = bitcoin.Transaction.fromHex(await (0, mempool_1.getRawTxHex)(ordinalUtxoTxId));
    ...
    async function verifySignedListingPSBTBase64(req, feeProvider, itemProvider) {
        ...
        const sellerAddressFromPSBT = bitcoin.address.fromOutputScript(bitcoin.Transaction.fromHex(await (0, mempool_1.getRawTxHex)((0, util_1.generateTxidFromHash)(psbt.txInputs[0].hash))).outs[psbt.txInputs[0].index].script, network);
...
(function (BuyerSigner) {
    ...
    async function doesUtxoContainInscription(utxo, itemProvider) {
        ...
        const tx = await (0, mempool_1.getRawTxVerbose)(utxo.txid);
        let foundInscription = false;
        for (const input of tx.vin) {
            if ((await (0, mempool_1.getRawTxVerbose)(input.txid)).status.confirmed) {
    ...
    async function generateUnsignedBuyingPSBTBase64(listing) {
        ...
        if (!listing.buyer.buyerPaymentUTXOs ||
            listing.buyer.buyerPaymentUTXOs.length < 1) {
            throw new interfaces_1.InvalidArgumentError('Buyer address has not enough utxos.');
        }
        if (listing.buyer.takerFeeBp < 0 || listing.buyer.takerFeeBp > 1) {
            throw new interfaces_1.InvalidArgumentError('The takeFeeBp range should be [0,1].');
        }
        let totalInput = 0;
        // Add two dummyUtxos
        ...
        for (const utxo of listing.buyer.buyerPaymentUTXOs) {
            const tx = bitcoin.Transaction.fromHex(await (0, mempool_1.getRawTxHex)(utxo.txid));
            const input = {
                hash: utxo.txid,
                index: utxo.vout,
